...
df = df.dropna(subset=["trailing_pe","profit_margin_raw","sector","ticker"]).copy()

df["profit_margin_pct"] = np.where(
    df["profit_margin_raw"].abs().median() <= 1.5,
    df["profit_margin_raw"]*100,
...
for i, sec in enumerate(sectors):
    ...
    ax.axvline(med_pe, ls="--", c="magenta", lw=1.5)
    ax.axhline(med_pm, ls="--", c="magenta", lw=1.5)
    ax.axvline(mean_pe, ls=":", c="royalblue", lw=1.5)
    ax.axhline(mean_pm, ls=":", c="royalblue", lw=1.5)
...
fig.suptitle("P/E vs Profit Margin by sectors", y=0.995, fontsize=14)
fig.text(0.5, 0.04, "Trailing P/E", ha="center")
